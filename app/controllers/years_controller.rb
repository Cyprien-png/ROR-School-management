class YearsController < ApplicationController
  ...
  before_action :set_deleted_year, only: [:undelete]
  ...
  def deleted
    @years = Year.with_deleted.where(isDeleted: true)
    render :deleted
  end

  def undelete
    @year.update(isDeleted: false)
    redirect_to years_path, notice: "Academic year was successfully restored."
  end
  ...
  private
    ...
    def set_deleted_year
      @year = Year.with_deleted.find(params[:id])
    end
